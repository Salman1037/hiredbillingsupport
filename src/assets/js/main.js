...
(function($) {
  ...
  "Customer Support Experts"
    ...
    let i = 0;
    const DURATION = 2200;      // time each word stays visible
    const TRANSITION_MS = 350;  // must match CSS transition

    setInterval(() => {
      // start fade/slide out
      el.classList.add("is-leaving");

      // after transition, swap text and fade/slide back in
      setTimeout(() => {
        i = (i + 1) % words.length;
